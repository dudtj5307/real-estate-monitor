...
from src.naver import Article, NaverClient, RateLimited
...
def client(**kw) -> NaverClient:
    # 실제 간격(1분/3분)으로 돌리면 테스트가 몇 분 걸린다
    return NaverClient(warmup_delay=0, page_delay=0,
                       retry_waits=(0.01, 0.02, 0.03), **kw)


def run_case(name: str, fail_times: int, *, budget: float = 60.0) -> int:
...
def test_warmup_429_retries():
    """워밍업 GET 의 429 도 같은 재시도 경로를 타야 한다.
    ...
    print(f"  워밍업 429: 재시도 경로 진입 (워밍업 {seen['n']}회)")


def test_timeout_is_retried():
    """차단은 429 가 아니라 무응답으로 나타나기도 한다 (DESIGN.md 1.5)."""
    c = client()
    hits = {"n": 0}
    ...
    assert "무응답" in message, message
    print(f"  타임아웃 재시도: 호출 {hits['n']}회")
