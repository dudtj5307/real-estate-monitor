...
| **하루 여러 번 실행** | **IP 차단** — 실행마다 러너 IP 가 새로 배정된다 | `daily.yml` cron 7개 + `--skip-if-done` |

시도 횟수를 3 → 7로 늘린 근거는, 막힌 실행이 이제 1분 안에 끝나기 때문이다.
실패가 싸지면 더 자주 던져도 된다. 예약 실행은 시작 전에 0~4분을 무작위로 쉬어
(`daily.yml`) 전 세계 cron 이 몰리는 정각 창을 피한다.
...
- 아침에 A 는 됐고 B 가 막혔다면, 점심 실행은 **B 만** 부른다. 요청이 줄면 그 새
  IP 가 차단에 걸릴 확률도 준다.
...
| 2 | 실행 IP 차단 | ⚠️ 경고만 — 다음 예약 실행이 다른 IP 로 재시도 |

차단은 **예상된 상황**이라 job 을 빨간 X 로 만들지 않는다. 하루 7번 중 5번이
빨갛게 뜨면 정작 진짜 고장을 놓치게 된다.
...
1. **하루 7번 실행**(KST 08:37 / 09:09 / 10:43 / 12:23 / 15:17 / 19:51 / 22:29)해
   매번 다른 러너 IP 를 받는다. 한 번 성공하면 나머지는 `--skip-if-done` 으로 즉시
   끝나고, 막힌 실행도 1분 안에 끝난다(§1.5). 정각을 피한 것도 같은 이유 —
   정각은 전 세계 워크플로가 몰려 IP 오염이 가장 심하다.
2. 그래도 매일 막히면 **self-hosted 러너**를 집 PC 에 올린다. 워크플로·스케줄·
   대시보드가 그대로인 채 실행 IP 만 집 IP 가 된다 (README "방법 C").
3. Actions 를 아예 접고 **작업 스케줄러로 전환**한다(`scripts/run_daily.ps1`).
   스크립트가 같아 전환 비용이 사실상 없다.

셋 다 실행 IP 를 바꾸는 방법이라는 점이 핵심이다. 헤더·쿠키·간격으로 푸는 길은
실측으로 막혀 있음이 확인됐다(§1.5). 프록시로 IP 만 갈아 끼우는 방법도 있지만,
데이터센터 대역은 애초에 차단 대상이라 러너 IP 와 같은 처지이고, 이 설계의
"비용 0원 · 자격증명 없음" 전제를 깬다.

- cron 7개 (UTC): `37 23` / `9 0` / `43 1` / `23 3` / `17 6` / `51 10` / `29 13`
- 예약 실행은 시작 전 0~4분 무작위 대기 — cron 정각에 몰리지 않게
