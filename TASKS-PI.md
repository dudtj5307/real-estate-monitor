...
T6  버튼 링크 ✅
T7  문구·정책 수정 ✅
T10 (선택) 버튼 실행 텔레그램 억제 ✅ A 안 채택 (코드 변경 없음)
T11 (향후) GPIO 웨이크              ← 하드웨어가 붙은 뒤
```

**코드·문서 작업은 전부 끝났습니다.** 남은 것은 사람이 해야 하는 일입니다.

1. **Pi 설치** — [scripts/pi/README.md](scripts/pi/README.md) 를 따라. §5(집 IP 검증)에서
   429 가 나오면 거기서 멈추고 재검토해야 합니다.
2. Pi 가 붙기 전까지는 **매일 14:00 에 watchdog 경보 + 폴백 수집**이 돕니다.
   시끄럽지만 데이터는 끊기지 않는 상태입니다.

**실기에서만 확인 가능한 것** (설치할 때 같이 볼 것)

- [ ] `systemd-analyze verify /etc/systemd/system/naver-monitor-poll.{service,timer}`
- [ ] `poll.sh` 의 `flock` 이 실제로 동시 실행을 막는다 (Windows 에 `flock` 이 없어
      스텁으로 분기만 검증했습니다)
- [ ] 첫 Pi push 에 `notify.yml` 이 실제로 트리거된다 (`paths` 필터 · deploy key)
...
## T9 — 기존 문서 갱신 `README.md`, `DESIGN.md` ✅ 완료

> **구현 결과 — 표에 없던 곳도 손댔습니다**
> - **`DESIGN.md` §1.5 를 최소한으로 건드렸습니다.** "§1 은 건드리지 말 것"이 원칙이지만
>   §1.5 의 대응 표와 종료코드 표는 실측이 아니라 **삭제된 `daily.yml` 을 가리키는
>   서술**이었습니다. 실측 숫자(429 지속시간·페이싱·0.03초 즉답)는 그대로 두고,
>   "하루 7번" 대목은 지우는 대신 **"당시엔 왜 7번이 옳았는지"를 남기고 지금 전제와
>   대비**시켰습니다. 그게 이 프로젝트에서 가장 값진 기록이기 때문입니다.
> - `DESIGN.md` §2.0.2 — 버튼이 "Actions 가 수집한다"고 설명하고 있었습니다.
>   이제 요청만 발행하므로 고쳤고, 최대 7분 지연을 명시했습니다.
> - `DESIGN-PI.md:118` — 삭제된 "README 방법 C" 를 가리키던 링크를 고쳤습니다.
> - `README.md` 명령행 옵션 표에 `--outbox` 가 빠져 있어 추가했습니다.
>
> **방법 C(self-hosted 러너)는 삭제**하고, 대신 "왜 안 쓰는가"를 방법 B 아래
> 경고로 남겼습니다. 지우기만 하면 나중에 같은 아이디어가 다시 나옵니다.
...
- [x] "하루 7번" 서술이 남아 있지 않다 — `DESIGN.md:135` 의 "당시엔 7번이었다"
      대비 문장만 의도적으로 남겼다
- [x] `daily.yml` 참조가 남아 있지 않다 (`DESIGN-PI.md`·이 문서의 변경 이력 서술 제외)
- [x] 새로 온 사람이 README 만 읽고 구조를 이해할 수 있다 — 첫 문단과 "방법 A" 에
      다이어그램을 넣고 설치는 `scripts/pi/README.md` 로 넘긴다

---

## T10 — (선택) 버튼 실행의 텔레그램·스냅샷 정책 ✅ **A 안 채택 (2026-08-09)**

> **결정: A(현행 유지) — 버튼 실행도 리포트를 보내고 스냅샷을 저장합니다.**
> 코드 변경 없음. `poll.sh` 의 `workflow_dispatch` 분기는 플래그 없이 그대로 둡니다.
>
> 근거: 정보 완전성이 알림 소음보다 중요합니다. 버튼을 누른 그 시점에 신규·변동이
> 바로 전달되고, 스냅샷이 저장되므로 다음 날 아침 리포트와도 어긋나지 않습니다.
>
> 나중에 알림이 성가시면 C 로 바꾸면 됩니다 — `poll.sh` 의 플래그 한 줄이고 새 코드는
> 필요 없습니다. **B(전송만 끄기)는 어떤 경우에도 채택하지 마세요** (아래 함정).
...
**결정 완료 (2026-08-09): A.** 정보 완전성이 알림 소음보다 우선한다는 판단입니다.
바꾸고 싶어지면 `poll.sh` 의 `workflow_dispatch` 분기에 `--dry-run --no-save` 를
붙이면 C 가 됩니다. **B 는 어떤 경우에도 채택하지 마세요.**
